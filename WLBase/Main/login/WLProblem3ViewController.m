...
#import "WLProblem3ViewController.h"

#define KCellHeight3 120.0/375*WIDTH

@interface WLProblem3ViewController ()<UITextFieldDelegate>

@end

@implementation WLProblem3ViewController
...
-(void)createUI{
    ...
    [self createLeft];
    [self createBall];
//    NSArray *temp1 =[[NSArray alloc] initWithObjects:@"新密码",@"新密码", nil];
//    NSArray *temp2 =[[NSArray alloc] initWithObjects:@"请输入新密码",@"请再次输入新密码",  nil];
    NSArray *temp1 =[[NSArray alloc] initWithObjects:@"新密码", nil];
    NSArray *temp2 =[[NSArray alloc] initWithObjects:@"请输入新密码",  nil];
    ...
    for (int i=0; i<3; i++) {
        ...
        if (i<2) {
            ...
            if (i<=select) {
                ...
                if (i<=(select-1)) {
                    lbOfLine.frame=CGRectMake(CGRectGetMaxX(viewOfBall.frame)+WLsize(5), viewOfBall.frame.origin.y+ballD/2.0-4.0/2,linew-WLsize(10.0), 4.0);
                    
                }
            }
            
            if (i<select) {
                lbOfLine.backgroundColor=selectBackgroundColor;
